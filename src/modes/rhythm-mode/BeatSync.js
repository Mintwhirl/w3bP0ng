...
export class BeatSync {
    ...
    constructor(bpm, duration, timing = DEFAULT_BEAT_TIMING) {
        ...
        Object.defineProperty(this, "beatTimestamps", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
     ...
     */
    checkHitTiming(hitTime, currentBeat) {
     ...
     */
    isInHitWindow(elapsedTime) {
        const currentBeat = this.getCurrentBeat(elapsedTime);
        if (currentBeat >= this.beatTimestamps.length)
            return false;
        const beatTime = this.beatTimestamps[currentBeat];
        if (beatTime === undefined)
            return false;
 ...
 */
export function createDefaultTrack(difficulty) {
    const configs = {
        easy: {
