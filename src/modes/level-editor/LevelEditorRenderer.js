...
export class LevelEditorRenderer {
    ...
    renderPlacedObject(obj) {
        ...
        switch (type) {
            ...
            case 'portal-cyan':
            case 'portal-magenta':
                this.renderPortal(data, selected);
                break;
            case 'bounce-pad':
                this.renderBouncePad(data, selected);
                break;
            case 'gravity-zone':
                this.renderGravityZone(data, selected);
                break;
            case 'paddle':
                this.renderPaddle(data, selected);
                break;
        }
    ...
    renderBouncePad(bouncePad, selected) {
    ...
    renderGravityZone(gravityZone, selected) {
    ...
    // PREVIEW RENDERING
    // ═══════════════════════════════════════════════════════════
    renderPreviewObject(state, mousePos) {
    ...
    renderPlacedObjectWithStyle(obj, style) {
        const { type, data } = obj;
        this.ctx.save();
    ...
    // ═══════════════════════════════════════════════════════════
    drawGlassPanel(x, y, width, height, color, opacity = 0.6) {
    ...
    drawSelectionOutline(obj) {
        ...
        this.ctx.setLineDash([5, 5]);
        this.ctx.strokeRect(obj.position.x - 5, obj.position.y - 5, 10, 10);
        this.ctx.setLineDash([]);
    }
}
// ═══════════════════════════════════════════════════════════
// HELPER FUNCTIONS
// ═══════════════════════════════════════════════════════════
function createTempPlacedObject(tool, position) {
...
function checkObjectOverlap(obj, existingObjects) {
