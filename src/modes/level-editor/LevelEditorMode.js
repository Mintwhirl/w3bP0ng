...
import { createInitialEditorState, placeObject, deleteObject, selectObject, moveObject, canPlaceObject, getObjectsAtPosition, undo, redo, } from './LevelEditorEngine';
import { editorStateToLevel, levelToCustomLevel, } from './LevelEditorEngine';
import { saveLevel, loadLevel, deleteLevel, } from './LevelData';
export default function LevelEditorMode() {
    const canvasRef = useRef(null);
    const rendererRef = useRef(null);
    const animationFrameRef = useRef(null);
    const [editorState, setEditorState] = useState(() => createInitialEditorState(800, 600));
    ...
    const handleMouseMove = useCallback((event) => {
        ...
        if (isDragging && draggedObjectId && dragStart) {
            ...
            setEditorState((prevState) => moveObject(prevState, draggedObjectId, {
                x: worldPos.x,
                y: worldPos.y,
            }));
        ...
        switch (editorState.mode) {
            case 'place':
                if (canPlaceObject(editorState.currentTool, editorState)) {
                    setEditorState((prevState) => placeObject(prevState, editorState.currentTool, worldPos));
                }
                break;
            case 'select':
                if (objectsAtPos.length > 0) {
                    ...
                    setDragStart(worldPos);
                    setIsDragging(true);
                    setEditorState((prevState) => selectObject(prevState, selectedObject.id, event.shiftKey));
                }
                else {
                    // Deselect all
            ...
            case 'delete':
                if (objectsAtPos.length > 0) {
                    ...
                    setEditorState((prevState) => deleteObject(prevState, objectToDelete.id));
                }
                break;
        }
    ...
    const handleLoadLevel = useCallback((name) => {
        try {
    ...
    const handleDeleteLevel = useCallback((name) => {
    ...
    return (_jsxs("div", { className: "level-editor-mode", children: [_jsx(ParticleBackground, {}), _jsx("canvas", { ref: canvasRef, width: 800, height: 600, className: "editor-canvas" }), _jsx(LevelUI, { editorState: editorState, onModeChange: handleModeChange, onToolChange: handleToolChange, onSaveLevel: handleSaveLevel, onLoadLevel: handleLoadLevel, onDeleteLevel: handleDeleteLevel, onTestLevel: handleTestLevel, onResetLevel: handleResetLevel, onGridToggle: handleGridToggle, onUndo: handleUndo, onRedo: handleRedo, canUndo: canUndo, canRedo: canRedo, isTestMode: editorState.isTestMode, testModeStats: editorState.testModeState ? {
                    balls: editorState.testModeState.balls.length,
                    hits: 0, // Would track hits in test mode
                    time: '0:00', // Would track time in test mode
                } : undefined }), _jsx("style", { jsx: true, children: `
        .level-editor-mode {
          position: relative;
          width: 100vw;
          height: 100vh;
...
function createInitialBall(x, y) {
    return {
        x,
        y,
