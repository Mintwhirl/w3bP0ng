 ...
 */
export function renderBattleRoyale(ctx, state) {
    try {
        if (!ctx || !state) {
 ...
 */
function renderBackground(ctx, width, height, effects) {
 ...
 */
function renderBeatSyncedParticles(ctx, width, height, intensity, beatProgress) {
 ...
 */
function renderArena(ctx, width, height, _state, // State parameter for future use
beatProgress) {
 ...
 */
function renderPaddle(ctx, player) {
    ...
    // Specular highlight
    const highlightGradient = ctx.createLinearGradient(paddle.x, paddle.y, paddle.x, paddle.y + paddle.height);
 ...
 */
function renderEliminatedPaddle(ctx, player) {
    const paddle = player.paddle;
    ctx.save();
    ctx.globalAlpha = 0.3;
 ...
 */
function renderBalls(ctx, balls) {
    try {
...
function renderBall(ctx, ball) {
    ...
    ctx.fill();
    // Inner highlight
    const highlightGradient = ctx.createRadialGradient(ball.x - ball.radius * 0.3, ball.y - ball.radius * 0.3, 0, ball.x, ball.y, ball.radius);
 ...
 */
function renderDynamicEffects(ctx, state) {
 ...
 */
function renderScreenGlow(ctx, intensity, beatProgress) {
 ...
 */
function renderFinalePulse(ctx, beatProgress, intensity) {
    const width = ctx.canvas.width;
    const height = ctx.canvas.height;
    ctx.save();
...
function renderHUD(ctx, state) {
    ...
    switch (game.gamePhase) {
        case 'opening':
            phaseText = 'CHAOS BEGINS';
            break;
        case 'midgame':
            phaseText = 'SURVIVAL';
            break;
        case 'finale':
            if (game.finalePhase === 'duel')
                phaseText = 'FINAL DUEL';
            if (game.finalePhase === 'victory')
                phaseText = 'VICTORY!';
            break;
    }
    if (phaseText) {
 ...
 */
export function calculateVisualEffects(state, beatProgress) {
 ...
 */
export function createRenderState(game, beatProgress) {
    ...
    const sortedPlayers = [...game.players]
        ...
        player,
        rank: index + 1,
        eliminationTime: !player.alive ? game.eliminations.find(e => e.playerId === player.id)?.timestamp : undefined,
    }));
    return {
        game,
        visualEffects,
