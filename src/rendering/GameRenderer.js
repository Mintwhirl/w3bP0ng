...
export class GameRenderer {
    ...
    renderBackground() {
        ...
        const gradientObj = this.ctx.createLinearGradient(0, 0, this.canvas.width, this.canvas.height);
        if (animated) {
            ...
            gradient.forEach((stop) => {
                const hueShift = stop.position === 0
                    ? sinTime * 5
                    : stop.position === 0.5
                        ? cosTime * 5
                        : halfSinTime * 5;
                gradientObj.addColorStop(stop.position, `hsl(${stop.hue + hueShift}, ${stop.saturation}%, ${stop.lightness}%)`);
            });
        }
        else {
            gradient.forEach((stop) => {
                gradientObj.addColorStop(stop.position, `hsl(${stop.hue}, ${stop.saturation}%, ${stop.lightness}%)`);
            });
        }
     ...
     */
    renderPaddles(paddles, bigPaddlePowerUp) {
        this.renderPaddle(paddles.left, 'left', this.theme.paddle.left, bigPaddlePowerUp);
        this.renderPaddle(paddles.right, 'right', this.theme.paddle.right, bigPaddlePowerUp);
    }
    /**
     * Render single paddle
     */
    renderPaddle(paddle, player, colors, bigPaddlePowerUp) {
        this.ctx.save();
     ...
     * Render main ball
     */
    renderBall(ball, gameWinner) {
        if (gameWinner)
            return; // Don't render ball when game is over
        this.ctx.save();
        // Outer glow
        this.ctx.shadowColor = this.theme.ball.shadowColor;
    ...
    renderMultiBalls(extraBalls) {
        ...
        extraBalls.forEach((extraBall) => {
            ...
            this.ctx.fillStyle = '#f8fafc';
            this.ctx.beginPath();
            this.ctx.arc(extraBall.x, extraBall.y, extraBall.radius * 0.6, 0, Math.PI * 2);
            this.ctx.fill();
            this.ctx.restore();
        });
    }
    ...
    renderWinMessage(winner) {
        ...
        this.ctx.textAlign = 'center';
        this.ctx.fillText(`${winner.toUpperCase()} PLAYER WINS!`, this.canvas.width / 2, this.canvas.height / 2);
    ...
    renderFallback() {
        ...
        this.ctx.textAlign = 'center';
        this.ctx.fillText('Game temporarily unavailable', this.canvas.width / 2, this.canvas.height / 2);
    }
    /**
     * Get current theme
     */
